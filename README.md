...
AWS Lambda wrapper for [fhir-client-py](https://github.com/apervita/fhir-client-py) which is a fork of [client-py](https://github.com/smart-on-fhir/client-py) which provides a [Python](https://www.python.org/) client for connecting to and interacting with a [FHIR](https://www.hl7.org/fhir/overview.html) server.

## Getting Started

### Prerequisites

AWS Lambda supports Python 3.8 as a runtime environment.
...
Pull down repo and run unittest tests from IntelliJ or CLI:

```
$ ./venv/bin/activate
$ export PYTHONPATH=$(pwd):$(pwd)/fhirclient
$ cd fhirclient/
$ python -m unittest lambda_handler_tests.py
...
----------------------------------------------------------------------
Ran 3 tests in 2.143s
...
`deploy.sh` is provided as a helper to show how re-deployments can be done to an existing Lambda function.

```
$ ./deploy.sh
...
upload: ./fhir-client-py.zip to s3://sandbox-fhir-cql/lambda/fhir-client-py.zip
{
    "FunctionName": "fhir-client-py",
    "FunctionArn": "arn:aws:lambda:us-east-1:757160032533:function:fhir-client-py",
    "Runtime": "python3.8",
    "Role": "arn:aws:iam::757160032533:role/service-role/fhir-client-py-role-m1t8jbcu",
    "Handler": "fhirclient.lambda_handler.lambda_handler",
    "CodeSize": 14464316,
    "Description": "",
    "Timeout": 60,
    "MemorySize": 128,
    "LastModified": "2021-04-06T14:59:16.226+0000",
    "CodeSha256": "tfqergKSZH9wfS75GDxBjzh3xZgPraKExhEG0IgHNA4=",
    "Version": "$LATEST",
    "TracingConfig": {
        "Mode": "PassThrough"
    },
    "RevisionId": "3ebfe0eb-07fd-47ca-9533-e0d0785f9b3e",
    "State": "Active",
...
AWS Lambda supports Python 3.8 runtime.
...
* **Greg Michalopoulos** - *Initial work*dle](https://gradle.org/) - Installation not required, it will donwload when you run `gradlew`.

## Authors

* **Greg Michalopoulos** - *Initial work*